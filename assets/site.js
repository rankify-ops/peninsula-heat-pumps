  ...
  document.querySelectorAll('.qform').forEach(function(qform){
    ...
    var sendQuote = function(cb){
      ...
      var autoResponse =
        ...
        'Need urgent help in the meantime? Call us on 0422 946 213 (Mon–Fri 8am–5pm).\n\n' +
        'Cheers,\n' +
      ...
      .then(function(res){
        ...
        if(res.ok && res.j && (res.j.success === true || res.j.success === 'true' || /success/i.test(res.j.message||''))){
          ...
          showError('Sorry, we couldn\'t send that automatically. Please call <a href="tel:0422946213" style="color:#dc2626;font-weight:700">0422 946 213</a> or email <a href="mailto:'+to+'" style="color:#dc2626;font-weight:700">'+to+'</a>.');
          cb(false);
        }
      })
      .catch(function(){
        if(btn){ btn.disabled = false; btn.textContent = origText; btn.style.opacity = ''; }
        showError('Network issue. Please call <a href="tel:0422946213" style="color:#dc2626;font-weight:700">0422 946 213</a> or email <a href="mailto:'+to+'" style="color:#dc2626;font-weight:700">'+to+'</a>.');
        cb(false);
      });
    };
